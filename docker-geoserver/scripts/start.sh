...
if [[ -z ${COMMUNITY_EXTENSIONS} ]]; then
  ...
  if  [[ ${FORCE_DOWNLOAD_COMMUNITY_EXTENSIONS} =~ [Tt][Rr][Uu][Ee] ]];then
    ...
    for plugin in $(cat /community_plugins/community_plugins.txt); do
      community_plugins_url="https://build.geoserver.org/geoserver/${GS_VERSION:0:4}.x/community-latest/geoserver-${GS_VERSION:0:4}-SNAPSHOT-${plugin}.zip"
    ...
    for ext in $(echo "${COMMUNITY_EXTENSIONS}" | tr ',' ' '); do
        ...
        if [[ ! -f /community_plugins/${ext}.zip ]]; then
          community_plugins_url="https://build.geoserver.org/geoserver/${GS_VERSION:0:4}.x/community-latest/geoserver-${GS_VERSION:0:4}-SNAPSHOT-${ext}.zip"
